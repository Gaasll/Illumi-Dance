...
function updateFirebaseFromModel(model) {
  function fireBaseObsACB(payload) {
    ...
    if (payload && payload.emotions)
      firebase
        .database()
        .ref(REF + "/" + user.uid + "/emotions")
        .set(payload.emotions);

    if (payload && payload.lastPlayed)
      firebase
        .database()
        .ref(REF + "/" + user.uid + "/lastPlayed")
        .set(payload.lastPlayed);
  }
...
function updateModelFromFirebase(model) {
  ...
  firebase
    .database()
    .ref(REF + "/" + user.uid + "/emotions")
    .on("child_added", function emotionsUpdatedInFirebase(firebaseData) {
  ...
  firebase
    .database()
    .ref(REF + "/" + user.uid + "/lastPlayed")
    .on("value", function lastPlayedUpdatedInFirebase(firebaseData) {
